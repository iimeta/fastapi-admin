...
type InviteRelation struct {
	...
	Status        int    `bson:"status,omitempty"`          // 邀请关系状态。
	...
	CreatedAt     int64  `bson:"created_at,omitempty"`      // 创建时间戳，毫秒。
	UpdatedAt     int64  `bson:"updated_at,omitempty"`      // 更新时间戳，毫秒。
}

// InviteReward 邀请收益数据写入对象。
type InviteReward struct {
	...
	Status          int    `bson:"status,omitempty"`           // 邀请收益状态。
	TriggerType     string `bson:"trigger_type,omitempty"`     // 收益触发类型。
	...
	CreditedAt      int64  `bson:"credited_at,omitempty"`      // 入账时间戳，毫秒。
...
// InviteRewardApply 邀请收益入账申请数据写入对象。
type InviteRewardApply struct {
	...
	Status       int      `bson:"status,omitempty"`         // 入账申请状态。
	AuditRole    string   `bson:"audit_role,omitempty"`     // 审核人角色。
	...
	DealRecordId string   `bson:"deal_record_id,omitempty"` // 审核通过后生成的财务流水ID。
	AppliedAt    int64    `bson:"applied_at,omitempty"`     // 申请时间戳，毫秒。
	AuditedAt    int64    `bson:"audited_at,omitempty"`     // 审核时间戳，毫秒。
	CreditedAt   int64    `bson:"credited_at,omitempty"`    // 入账时间戳，毫秒。
	CreatedAt    int64    `bson:"created_at,omitempty"`     // 创建时间戳，毫秒。
	UpdatedAt    int64    `bson:"updated_at,omitempty"`     // 更新时间戳，毫秒。
}
