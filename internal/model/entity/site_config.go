...
type SiteConfig struct {
	...
	InviteIpDailyLimit                int                   `bson:"invite_ip_daily_limit,omitempty"`                 // 同IP每日邀请注册上限
	InviteIpTotalLimit                int                   `bson:"invite_ip_total_limit,omitempty"`                 // 同IP累计邀请注册上限
	InviteIpPerInviterLimit           int                   `bson:"invite_ip_per_inviter_limit,omitempty"`           // 同IP+同邀请人注册上限
	InviteIpLimitAction               string                `bson:"invite_ip_limit_action,omitempty"`                // IP限制触发动作[block:拒绝注册, silent:允许注册但不发放额度]
