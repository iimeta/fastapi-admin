...
type Ticket struct {
	...
	AssigneeId     int    `bson:"assignee_id,omitempty"`     // 处理人ID
	AssigneeRole   string `bson:"assignee_role,omitempty"`   // 处理人角色[reseller:代理商, admin:管理员]
	ReplyCount     int    `bson:"reply_count,omitempty"`     // 回复数量
	LastReplyAt    int64  `bson:"last_reply_at,omitempty"`   // 最后回复时间
	UserNotice     bool   `bson:"user_notice,omitempty"`     // 提交方提醒
	AssigneeNotice bool   `bson:"assignee_notice,omitempty"` // 处理方提醒
