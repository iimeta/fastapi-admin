...
func (s *sModelAgent) Page(ctx context.Context, params model.ModelAgentPageReq) (*model.ModelAgentPageRes, error) {
	...
	results, err := dao.ModelAgent.FindByPage(ctx, paging, filter, &dao.FindOptions{SortFields: []string{"status", "-created_at"}})
	if err != nil {
		logger.Error(ctx, err)
		return nil, err
	}
...
func (s *sModelAgent) QuickFillModel(ctx context.Context, params model.ModelAgentQuickFillModelReq) ([]string, error) {
	...
	if _, _, err := sutil.HttpGet(ctx, fmt.Sprintf("%s://%s/v1/models", baseUrl.Scheme, baseUrl.Host), g.MapStrStr{"Authorization": "Bearer " + keys[0]}, nil, &result, config.Cfg.Http.Timeout*time.Second, config.Cfg.Http.ProxyUrl, nil); err != nil {
		logger.Error(ctx, err)
...
func (s *sModelAgent) TestModel(ctx context.Context, params model.ModelAgentTestModelReq) (*model.ModelAgentTestModelRes, error) {
	...
	if provider.Code != sconsts.PROVIDER_OPENAI {
		options.ReqPassthroughParams = []string{"req_data"}
	}
