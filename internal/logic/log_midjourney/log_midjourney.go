...
type sLogMidjourney struct{}

func init() {
	service.RegisterLogMidjourney(New())
}

func New() service.ILogMidjourney {
	return &sLogMidjourney{}
}

// Midjourney日志详情
func (s *sLogMidjourney) Detail(ctx context.Context, id string) (*model.LogMidjourney, error) {

	result, err := dao.LogMidjourney.FindById(ctx, id)
	if err != nil {
...
// Midjourney日志分页列表
func (s *sLogMidjourney) Page(ctx context.Context, params model.LogMidjourneyPageReq) (*model.LogMidjourneyPageRes, error) {
	...
	results, err := dao.LogMidjourney.FindByPage(ctx, paging, filter, &dao.FindOptions{SortFields: []string{"-req_time", "status", "-created_at"}})
	if err != nil {
		logger.Error(ctx, err)
		return nil, err
	}

	items := make([]*model.LogMidjourney, 0)
	...
	return &model.LogMidjourneyPageRes{
		Items: items,
		Paging: &model.Paging{
			Page:     paging.Page,
