...
// 绘图日志详情
func (s *sLogImage) Detail(ctx context.Context, id string) (*model.LogImage, error) {

	result, err := dao.LogImage.FindById(ctx, id)
	if err != nil {
...
// 绘图日志分页列表
func (s *sLogImage) Page(ctx context.Context, params model.LogImagePageReq) (*model.LogImagePageRes, error) {
	...
	findOptions := &dao.FindOptions{
		...
		IncludeFields: []string{"_id", "user_id", "app_id", "creator", "model", "model_type", "prompt", "size", "action", "stream", "image_data.url", "spend", "conn_time", "duration", "total_time", "req_time", "status", "internal_time", "receive_time", "is_smart_match", "provider_name", "provider_code", "err_msg"},
	}
...
// 绘图日志详情复制字段值
func (s *sLogImage) CopyField(ctx context.Context, params model.LogImageCopyFieldReq) (string, error) {

	result, err := dao.LogImage.FindById(ctx, params.Id)
	if err != nil {
		logger.Error(ctx, err)
		return "", err
	}
