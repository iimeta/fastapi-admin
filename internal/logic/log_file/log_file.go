...
// 文件日志批量操作
func (s *sLogFile) BatchOperate(ctx context.Context, params model.LogFileBatchOperateReq) error {
	...
	if err := grpool.AddWithRecover(gctx.NeverDone(ctx), func(ctx context.Context) {
		...
		case consts.ACTION_TIME:
			...
			if _, err := dao.LogFile.DeleteMany(ctx, filter); err != nil {
				logger.Error(ctx, err)
			}

		case consts.ACTION_DELETE:
			if _, err := dao.LogFile.DeleteMany(ctx, bson.M{"_id": bson.M{"$in": params.Ids}}); err != nil {
				logger.Error(ctx, err)
			}
		}
