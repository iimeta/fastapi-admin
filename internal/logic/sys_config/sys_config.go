...
func (s *sSysConfig) Update(ctx context.Context, params model.SysConfigUpdateReq) (newData *entity.SysConfig, err error) {
	...
	case "user_expire_task":
		sysConfig = &do.SysConfig{UserExpireTask: params.UserExpireTask}
...
func (s *sSysConfig) Detail(ctx context.Context) (*model.SysConfig, error) {
	...
	return &model.SysConfig{
		...
		UserExpireTask:            sysConfig.UserExpireTask,
...
func (s *sSysConfig) Reset(ctx context.Context, params model.SysConfigResetReq) (*entity.SysConfig, error) {
	...
	case "user_expire_task":
		sysConfigUpdateReq.UserExpireTask = s.Default().UserExpireTask
	case "model_agent_health_check_task":
...
func (s *sSysConfig) Init(ctx context.Context) (sysConfig *entity.SysConfig, err error) {
	...
	if sysConfig.UserExpireTask == nil {
		if sysConfig, err = s.Reset(ctx, model.SysConfigResetReq{Action: "user_expire_task"}); err != nil {
			logger.Error(ctx, err)
			return nil, err
		}
	}
...
func (s *sSysConfig) Default() *do.SysConfig {
	return &do.SysConfig{
		...
		UserExpireTask: &common.UserExpireTask{
			Open:        true,
			Cron:        "0 * * * * ?",
			LockMinutes: 10,
		},
		...
		ModelAgentSessionKeep: &common.ModelAgentSessionKeep{
			...
			AgentLimit:             10000,
			GlobalLimit:            100000,
			EnableSystemPromptHash: true,
			Rules: []common.SessionKeepRule{
				{
				...
				},
				{
					...
					KeySources: []common.SessionKeepKeySource{{Type: "body", Key: "prompt_cache_key"}},
					Transform:  "md5",
				},
			},
		},
		...
		Test: &common.Test{
			Tests: []common.TestItem{
				...
				},
				{
					...
					Model:       "all",
					ModelType:   2,
					RequestData: `{"model":"{{.model}}","prompt":"A cute orange cat","n":1,"size":"1024x1024"}`,
				},
				{
					Provider:    sconsts.PROVIDER_OPENAI,
					Model:       "all",
					ModelType:   5,
					RequestData: `{"model":"{{.model}}","input":"The quick brown fox jumped over the lazy dog.","voice":"alloy"}`,
				},
				{
					Provider:    sconsts.PROVIDER_OPENAI,
					Model:       "all",
					ModelType:   7,
					RequestData: `{"model":"{{.model}}","input":"The food was delicious and the waiter...","encoding_format":"float"}`,
				},
				{
					Provider:    sconsts.PROVIDER_OPENAI,
					Model:       "all",
