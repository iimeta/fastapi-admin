...
func (s *sNoticeTemplate) Default() []*do.NoticeTemplate {
	...
	noticeTemplates := []*do.NoticeTemplate{
		...
		{
			Name:     consts.SCENE[consts.SCENE_TICKET_CREATE],
			Scenes:   []string{consts.SCENE_TICKET_CREATE},
			Title:    `{{.action}}：{{.ticket.title}}`,
			Content:  `<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb;font-family:Arial,'Microsoft YaHei',sans-serif;line-height:1.6;color:#333"><div style="background:#2563eb;padding:24px;text-align:center;color:#fff"><span style="font-size:22px;font-weight:700">{{.action}}</span></div><div style="padding:28px"><p style="margin:0 0 14px">尊敬的 <strong>{{.name}}</strong>：</p><p style="margin:0 0 18px">工单 <strong>{{.ticket.ticket_no}}</strong> 已创建，请及时查看处理。</p><div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0"><p style="margin:0 0 8px"><strong>工单编号：</strong>{{.ticket.ticket_no}}</p><p style="margin:0 0 8px"><strong>标题：</strong>{{.ticket.title}}</p><p style="margin:0 0 8px"><strong>提交人：</strong>{{.ticket.user_name}}</p><p style="margin:0"><strong>当前状态：</strong>{{.ticket.status_name}}</p></div></div><div style="padding:18px;text-align:center;font-size:12px;color:#9ca3af;background:#f9fafb"><p style="margin:0 0 8px 0">此为系统邮件，请勿直接回复</p><p style="margin:0"><a href="{{.site.jump_url}}" style="color:#2563eb;text-decoration:none">{{.site.copyright}}</a></p></div></div>`,
		...
		{
			Name:     consts.SCENE[consts.SCENE_TICKET_REPLY],
			Scenes:   []string{consts.SCENE_TICKET_REPLY},
			Title:    `{{.action}}：{{.ticket.title}}`,
			Content:  `<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb;font-family:Arial,'Microsoft YaHei',sans-serif;line-height:1.6;color:#333"><div style="background:#2563eb;padding:24px;text-align:center;color:#fff"><span style="font-size:22px;font-weight:700">{{.action}}</span></div><div style="padding:28px"><p style="margin:0 0 14px">尊敬的 <strong>{{.name}}</strong>：</p><p style="margin:0 0 18px">工单 <strong>{{.ticket.ticket_no}}</strong> 有新的回复，请及时查看处理。</p><div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0"><p style="margin:0 0 8px"><strong>工单编号：</strong>{{.ticket.ticket_no}}</p><p style="margin:0 0 8px"><strong>标题：</strong>{{.ticket.title}}</p><p style="margin:0"><strong>当前状态：</strong>{{.ticket.status_name}}</p></div></div><div style="padding:18px;text-align:center;font-size:12px;color:#9ca3af;background:#f9fafb"><p style="margin:0 0 8px 0">此为系统邮件，请勿直接回复</p><p style="margin:0"><a href="{{.site.jump_url}}" style="color:#2563eb;text-decoration:none">{{.site.copyright}}</a></p></div></div>`,
			Channels: []string{consts.NOTICE_CHANNEL_EMAIL},
			IsPublic: true,
			Status:   1,
			UserId:   1,
		...
		{
			Name:     consts.SCENE[consts.SCENE_TICKET_STATUS],
			Scenes:   []string{consts.SCENE_TICKET_STATUS},
			Title:    `{{.action}}：{{.ticket.title}}`,
			Content:  `<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb;font-family:Arial,'Microsoft YaHei',sans-serif;line-height:1.6;color:#333"><div style="background:#2563eb;padding:24px;text-align:center;color:#fff"><span style="font-size:22px;font-weight:700">{{.action}}</span></div><div style="padding:28px"><p style="margin:0 0 14px">尊敬的 <strong>{{.name}}</strong>：</p><p style="margin:0 0 18px">工单 <strong>{{.ticket.ticket_no}}</strong> 状态已变更，请及时查看处理。</p><div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0"><p style="margin:0 0 8px"><strong>工单编号：</strong>{{.ticket.ticket_no}}</p><p style="margin:0 0 8px"><strong>标题：</strong>{{.ticket.title}}</p><p style="margin:0"><strong>当前状态：</strong>{{.ticket.status_name}}</p></div></div><div style="padding:18px;text-align:center;font-size:12px;color:#9ca3af;background:#f9fafb"><p style="margin:0 0 8px 0">此为系统邮件，请勿直接回复</p><p style="margin:0"><a href="{{.site.jump_url}}" style="color:#2563eb;text-decoration:none">{{.site.copyright}}</a></p></div></div>`,
			Channels: []string{consts.NOTICE_CHANNEL_EMAIL},
			IsPublic: true,
			Status:   1,
			UserId:   1,
		...
		{
			Name:     consts.SCENE[consts.SCENE_TICKET_AUTO_CLOSE],
			Scenes:   []string{consts.SCENE_TICKET_AUTO_CLOSE},
			Title:    `{{.action}}：{{.ticket.title}}`,
			Content:  `<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e5e7eb;font-family:Arial,'Microsoft YaHei',sans-serif;line-height:1.6;color:#333"><div style="background:#475569;padding:24px;text-align:center;color:#fff"><span style="font-size:22px;font-weight:700">{{.action}}</span></div><div style="padding:28px"><p style="margin:0 0 14px">尊敬的 <strong>{{.name}}</strong>：</p><p style="margin:0 0 18px">工单 <strong>{{.ticket.ticket_no}}</strong> 因超过 {{.auto_close_days}} 天未继续回复，系统已自动关闭。</p><div style="background:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0"><p style="margin:0 0 8px"><strong>工单编号：</strong>{{.ticket.ticket_no}}</p><p style="margin:0 0 8px"><strong>标题：</strong>{{.ticket.title}}</p><p style="margin:0"><strong>当前状态：</strong>{{.ticket.status_name}}</p></div><p style="margin:18px 0 0;color:#475569">如果问题仍未解决，您可以手动重新打开该工单后继续提问。</p></div><div style="padding:18px;text-align:center;font-size:12px;color:#9ca3af;background:#f9fafb"><p style="margin:0 0 8px 0">此为系统邮件，请勿直接回复</p><p style="margin:0"><a href="{{.site.jump_url}}" style="color:#2563eb;text-decoration:none">{{.site.copyright}}</a></p></div></div>`,
			Channels: []string{consts.NOTICE_CHANNEL_EMAIL},
			IsPublic: true,
			Status:   1,
			UserId:   1,
