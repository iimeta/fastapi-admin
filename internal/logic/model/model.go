...
func (s *sModel) Create(ctx context.Context, params model.ModelCreateReq) error {
	...
	m := &do.Model{
		...
		IsEnableDataPassthrough:  params.IsEnableDataPassthrough,
		ReqPassthroughParams:     params.ReqPassthroughParams,
		ReqHeaderPassthroughMode: params.ReqHeaderPassthroughMode,
		ReqHeaderPassthroughList: params.ReqHeaderPassthroughList,
		ResPassthroughParams:     params.ResPassthroughParams,
...
func (s *sModel) Detail(ctx context.Context, id string) (*model.Model, error) {
	...
	detail := &model.Model{
		...
		TimeRules:                common.ConvTimeRulesToPercent(m.TimeRules),
		Pricing:                  common.ConvModelPricingToPrice(m.Pricing),
		IsEnableDataPassthrough:  m.IsEnableDataPassthrough,
		ReqPassthroughParams:     m.ReqPassthroughParams,
		ReqHeaderPassthroughMode: m.ReqHeaderPassthroughMode,
		ReqHeaderPassthroughList: m.ReqHeaderPassthroughList,
		ResPassthroughParams:     m.ResPassthroughParams,
...
func (s *sModel) InitSync(ctx context.Context, params model.ModelInitSyncReq) error {
	...
	if _, _, err = sutil.HttpGet(ctx, rawURL, g.MapStrStr{"Authorization": "Bearer " + params.Key}, g.MapStrAny{"is_fastapi": true}, &result, config.Cfg.Http.Timeout*time.Second, config.Cfg.Http.ProxyUrl, nil); err != nil {
		logger.Error(ctx, err)
		return err
	}
	...
	for _, data := range result.Data {
		...
		if modelMap[data.Id] == nil {
			...
			modelUpdateReq := model.ModelUpdateReq{
				...
				Pricing:                  data.FastAPI.Pricing,
				IsEnableDataPassthrough:  detail.IsEnableDataPassthrough,
				ReqPassthroughParams:     detail.ReqPassthroughParams,
				ReqHeaderPassthroughMode: detail.ReqHeaderPassthroughMode,
				ReqHeaderPassthroughList: detail.ReqHeaderPassthroughList,
				ResPassthroughParams:     detail.ResPassthroughParams,
				ResHeaderPassthroughMode: detail.ResHeaderPassthroughMode,
